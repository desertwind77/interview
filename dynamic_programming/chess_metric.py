...
#
# Problem Statement
# Suppose you had an n by n chess board and a super piece called a kingknight. Using
# only one move the kingknight denoted 'K' below can reach any of the spaces denoted
# 'X' or 'L' below:
#    .......
#    ..L.L..
#    .LXXXL.
...
# (vertical, horizontal or diagonally) or can make an 'L' shaped move. An 'L' shaped
# move involves moving 2 spaces horizontally then 1 space vertically or 2 spaces
# vertically then 1 space horizontally. In the drawing above, the 'L' shaped moves
# are marked with 'L's whereas the one space moves are marked with 'X's. In addition,
# a kingknight may never jump off the board.
#
...
#
# Note, two ways of getting from start to end are distinct if their respective move
# sequences differ in any way. In addition, you are allowed to use spaces on the
# board (including start and finish) repeatedly during a particular path from start
# to finish. We will ensure that the total number of paths is less than or equal to
# 2^63-1 (the upper bound for a long).
#
# Definition
# Class:	ChessMetric
# Method: howMany
